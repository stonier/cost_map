...
TEST(checkGetCentroid, rectangle)
{
  ...
  cost_map::Position expectedCentroid(-0.5, 0.5);
  cost_map::Position centroid = rectangle.getCentroid();
  EXPECT_DOUBLE_EQ(expectedCentroid.x(), centroid.x());
  EXPECT_DOUBLE_EQ(expectedCentroid.y(), centroid.y());
}
...
TEST(checkConvexHull, createHull)
{
  ...
  cost_map::Polygon hull = cost_map::Polygon::convexHull(polygon1, polygon2);
...
TEST(checkConvexHullCircles, createHull)
{
  ...
  cost_map::Polygon hull = cost_map::Polygon::convexHullOfTwoCircles(center1, center2, radius);
  ...
  hull = cost_map::Polygon::convexHullOfTwoCircles(center1, center2, radius, nVertices);
...
TEST(checkConvexHullCircle, createHull)
{
  ...
  cost_map::Polygon hull = cost_map::Polygon::fromCircle(center, radius);
  ...
  hull = cost_map::Polygon::fromCircle(center, radius, nVertices);
...
{
  cost_map::Polygon polygon({Position(1.0, 1.0), Position(0.0, 0.0), Position(1.1, -1.1)});
  MatrixXd A;
  VectorXd b;
...
{
  cost_map::Polygon polygon({Position(-1.0, 0.5), Position(-1.0, -0.5), Position(1.0, -0.5)});
  MatrixXd A;
  VectorXd b;
  ASSERT_TRUE(polygon.convertToInequalityConstraints(A, b));
...
{
  cost_map::Polygon polygon({Position(1.0, 1.0), Position(0.0, 0.0), Position(1.0, -1.0)});
